"""教师逐字稿 Agent API：runs / messages。

- POST /courses/{course_id}/tasks/verbatim/runs —— 创建运行（initial 或带指令的 message）
- POST /courses/{course_id}/tasks/verbatim/messages —— 教师修改指令（携带章节作用域与 mode）
...
verbatim_router = APIRouter(tags=["verbatim-agent"])

TASK_TYPE = "verbatim"


class VerbatimMessageRequest(BaseModel):
...
@verbatim_router.post("/courses/{course_id}/tasks/verbatim/runs", status_code=202)
async def create_verbatim_run(
    course_id: str,
    payload: VerbatimMessageRequest,
...
@verbatim_router.post("/courses/{course_id}/tasks/verbatim/messages", status_code=202)
async def send_verbatim_message(
    course_id: str,
    payload: VerbatimMessageRequest,
    user: User = Depends(current_user),
    db: AsyncSession = Depends(get_db),
):
    return await create_verbatim_run(course_id, payload, user, db)
