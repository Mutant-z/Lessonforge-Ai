...
async def test_full_api_generation_and_export(client, auth_headers):
    ...
    for _ in range(200):
        ...
        if all(task["status"] in {"review", "approved", "stale", "failed", "ready_to_generate", "waiting_dependency"} for task in project["tasks"]):
            break
        await asyncio.sleep(0.02)
    assert all(task["status"] in {"review", "approved", "stale", "ready_to_generate", "waiting_dependency"} for task in project["tasks"]), project
