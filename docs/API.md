...
- `GET /ppt-templates`：读取内置 PPT 模板目录；`POST /artifacts/{id}/apply-template`：只切换视觉主题并创建新版本
...
PPT 模板应用请求为 `{ template_id, expected_version }`。重复应用当前模板返回 `changed: false` 且不创建版本；纯模板版本不会使视频脚本或逐字稿过期。模板 ID 无效返回 422，文件已锁定、Agent 正在运行或版本已变化返回 409。
