...
/** 课后练习 V2 工作台：左侧 Agent 执行时间线（意图/工具/QA/返修流式显示）+ 右侧试卷预览。
 *
 * 复用通用执行组件（AgentExecutionTimeline / AgentComposer / pipeline store），
 * 消息走 /courses/{course_id}/tasks/exercise/messages（携带分区作用域与 mode），
 * 运行中指令排队（instructions），人工确认从同一 GenerationRun checkpoint 恢复。
 */
...
import ExercisePreview from '../../domain/ExercisePreview.vue';
...
// 选中的练习分区（对应 selected_section_ids：三区固定 ID）
...
/** 人工确认从原 GenerationRun 恢复（同一 checkpoint 恢复语义，与 task_sheet 一致）。 */
async function handleHumanResponse(requestId: string, choice: string, data: Record<string, unknown> = {}) {
  ...
  try {
    const result = await pipelineApi.exerciseHumanResponse(props.courseId, runId, requestId, choice, data);
...
watch(() => status.value, async (newStatus, oldStatus) => {
  if (['queued', 'running', 'pausing', 'paused'].includes(newStatus)) {
    startPolling();
  } else {
    stopPolling();
    // Pipeline 完成后刷新任务，使右侧 ExercisePreview 展示最新版 artifact。
    // （exercise 没有流式草稿预览，current_artifact 必须通过任务刷新更新。）
    const wasRunning = ['queued', 'running', 'pausing', 'paused'].includes(oldStatus || '');
    const isTerminal = ['completed', 'review', 'stale', 'failed', 'cancelled'].includes(newStatus);
...
<template>
  <div ref="containerRef" class="exercise-workbench">
    ...
    >
      <div class="preview-scroll">
        <div v-if="!artifact" class="preview-empty">课后练习尚未生成，发送指令开始创作。</div>
        <ExercisePreview
          v-else-if="previewContent"
          :content="previewContent"
          :source-versions="sourceVersions"
        />
        <div v-else class="preview-empty">正在生成课后练习…</div>
      </div>
