...
/** 教学设计 V2 工作台：左侧 Agent 执行时间线（意图/工具/QA/返修流式显示）+ 右侧文档预览。
 *
 * 复用通用执行组件（AgentExecutionTimeline / AgentComposer / ModelSelector / pipeline store），
 * 消息走 /courses/{course_id}/tasks/lesson_plan/runs（携带章节作用域与 mode）。
 */
...
const resultStatus = computed(() => String(pipelineStore.run?.plan?.result_status || ''));
...
const statusType = computed(() => {
  ...
  if (status.value === 'completed') {
    if (resultStatus.value === 'rejected') return 'warning';
    if (resultStatus.value === 'no_change' || resultStatus.value === 'answer_only') return 'info';
    if (resultStatus.value === 'needs_confirmation') return 'warning';
    return 'success';
  }
...
const statusLabel = computed(() => {
  if (status.value === 'completed') {
    if (resultStatus.value === 'rejected') return '修改未应用';
    if (resultStatus.value === 'no_change') return '无变更';
    if (resultStatus.value === 'answer_only') return '仅回答未修改';
    if (resultStatus.value === 'needs_confirmation') return '待确认';
  }
  return PIPELINE_STATUS_LABELS[status.value] || status.value || '未运行';
});

// 选中的大纲章节（对应 selected_section_ids）
const selectedSectionIds = ref<string[]>([]);
const activeSectionId = ref<string | null>(null);
const mode = ref<LessonPlanMode>('auto');

const markdown = computed(() => task.value?.current_artifact?.content_markdown || null);
...
<template>
  <div ref="containerRef" class="lesson-plan-workbench">
    <!-- 移动端视口视图切换 Tab -->
