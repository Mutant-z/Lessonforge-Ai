...
import type { VideoAnimationAction, VideoPedagogicalAction, VideoPedagogicalRole, VideoScene, VideoScriptContent } from '../../types';
...
const pedagogicalActions: { value: VideoPedagogicalAction; label: string }[] = [
  { value: 'hook', label: '导入勾子' },
  { value: 'objective_guide', label: '目标指引' },
  { value: 'scenario_connect', label: '情境关联' },
  { value: 'metaphor_explain', label: '隐喻讲解' },
  { value: 'misconception_alert', label: '易错预警' },
  { value: 'step_demonstration', label: '步骤示范' },
  { value: 'check_in', label: '检查点' },
  { value: 'summary_recap', label: '总结升华' },
];
...
function narrationStats(scene: VideoScene) {
  const chars = scene.audio_track.narration_text.trim().length;
  const cps = scene.audio_track.speaking_rate_cps || 4.0;
  const allotted = Math.max(1, scene.end_seconds - scene.start_seconds);
  const estimated = chars / cps;
  const pace = chars / (allotted / 60); // 字/分钟
  return { chars, estimated, allotted, pace };
}

function paceWarning(scene: VideoScene) {
  const { pace, estimated, allotted } = narrationStats(scene);
  if (pace > 360) return `语速约 ${pace.toFixed(0)} 字/分，超过 360 字/分，建议拆分分镜或精简台词`;
  if (pace < 150) return `语速约 ${pace.toFixed(0)} 字/分，低于 150 字/分，本镜时长充裕`;
  if (estimated > allotted * 1.15) return `旁白预计 ${estimated.toFixed(1)}s，超过本镜 ${allotted.toFixed(1)}s，请缩短台词`;
  return '';
}

function recalculateSceneTimelines() {
  let cursor = 0;
  for (const scene of model.value.scenes) {
    const chars = scene.audio_track.narration_text.trim().length;
    const cps = scene.audio_track.speaking_rate_cps || 4.0;
    let speech = chars / cps;
    let pauses = scene.audio_track.pause_cues.reduce((sum, cue) => sum + cue.duration_seconds, 0);
    if (scene.interaction) pauses += scene.interaction.wait_seconds;
    const duration = Math.max(3, speech + pauses);
    scene.start_seconds = Math.round(cursor * 10) / 10;
    scene.end_seconds = Math.round((cursor + duration) * 10) / 10;
    cursor = scene.end_seconds;

    const chunkCount = scene.text_track.subtitle_chunks.length || 1;
    scene.text_track.subtitle_chunks.forEach((chunk, index) => {
      chunk.start_offset_seconds = Math.round((index * duration / chunkCount) * 100) / 100;
      chunk.end_offset_seconds = Math.round((Math.min(duration, (index + 1) * duration / chunkCount)) * 100) / 100;
    });
    scene.visual_track.animation_cues = scene.visual_track.animation_cues.filter(cue => cue.offset_seconds <= duration);
    scene.audio_track.pause_cues = scene.audio_track.pause_cues.filter(cue => cue.offset_seconds + cue.duration_seconds <= duration);
    scene.audio_track.sound_cues = scene.audio_track.sound_cues.filter(cue => cue.offset_seconds <= duration);
  }
  model.value.production_settings.target_duration_seconds = Math.round(cursor);
...
<template>
  <div class="video-script-editor">
    ...
    <section>
      <header><span>01</span><h3>结构化分镜</h3><el-button size="small" @click="recalculateSceneTimelines">按台词对齐时间轴</el-button><el-button :icon="Plus" size="small" @click="addScene">拆分末尾场景</el-button></header>
      <article v-for="(scene, sceneIndex) in model.scenes" :key="scene.id" class="scene-editor-card">
        ...
        <div class="track-editor-grid">
          ...
          <section class="track-editor">
            ...
            <p class="pace-hint" :class="{ warn: paceWarning(scene) }">
              字数 {{ narrationStats(scene).chars }} · 预计口播 {{ narrationStats(scene).estimated.toFixed(1) }}s ·
              本镜时长 {{ narrationStats(scene).allotted.toFixed(1) }}s · 语速 {{ narrationStats(scene).pace.toFixed(0) }}字/分
              <template v-if="paceWarning(scene)"><br />⚠️ {{ paceWarning(scene) }}</template>
            </p>
            <div class="field-grid two">
              <label><span>教学动作</span><el-select v-model="scene.audio_track.pedagogical_action"><el-option v-for="item in pedagogicalActions" :key="item.value" :label="item.label" :value="item.value" /></el-select></label>
              <label><span>讲述语气</span><el-input v-model="scene.audio_track.delivery_tone" /></label>
              <label><span>语速（字/秒）</span><el-input-number v-model="scene.audio_track.speaking_rate_cps" :min="2" :max="6" :step="0.5" /></label>
...
.pace-hint { margin: 6px 0 10px; color: #656a73; font-size: 11px; line-height: 1.5; }.pace-hint.warn { color: #c0392b; font-weight: 700; }
